...
def list_trades(
    ...
    account_id: Optional[UUID] = None,  # ⬅ filtre compte
    session: Session = Depends(get_session),
):
    stmt = select(Trade).order_by(Trade.trade_date.desc(), Trade.num_trade.desc())
    if date_from: stmt = stmt.where(Trade.trade_date >= date_from)
    if date_to:   stmt = stmt.where(Trade.trade_date <= date_to)
    if bias:      stmt = stmt.where(Trade.bias_daily == bias)
    if tags:      stmt = stmt.where(Trade.tags.ilike(f"%{tags}%"))
    if account_id: stmt = stmt.where(Trade.account_id == account_id)
...
def delete_trade(trade_id: UUID, session: Session = Depends(get_session)):
    ...
    if not trade:
        raise HTTPException(404)
    session.delete(trade); session.commit()
    return {"ok": True}

# --- Export CSV (avec filtres dont account_id) ---
...
def export_csv(
    ...
    writer.writerow([
        "id","account_id","num_trade","trade_date","analyse_time","bias_daily",
    ...
    for t in rows:
        writer.writerow([
            str(t.id), str(t.account_id) if t.account_id else "",
            t.num_trade, t.trade_date, t.analyse_time, t.bias_daily,
